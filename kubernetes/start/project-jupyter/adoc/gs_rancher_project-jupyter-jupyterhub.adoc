...
[IMPORTANT]
====
Any keys that do not have values assigned to them will be omitted when the ConfigMap is created. 
This missing data will cause errors later in this procedure.
====
+
...
====
+
.. When the {rancher-kubectl-shell} is ready, run the following command to ensure the fully qualified domain name of the {jupyterhub-deployed-name} service resolves to its dedicated IP address.
+
[IMPORTANT]
====
If the follow command fails with the error that the {rancher-kubectl-shell-variables-file} doesn't exist, return to the section <<Create the variables file>> to recreate it.
