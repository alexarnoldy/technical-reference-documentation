...
[NOTE]
====
If you prefer, update the cloud-config tempalte below to configure a more secure password for the root and sles users.
====
+
[IMPORTANT]
====
...
[TIP]
====
When selecting the `User Data` template, some browsers won't pull in the correct data if the `Install guest agent` checkbox is selected. To ensure a consistent experience, you may first unchecked that box, selected the correct user data, then checked that box again.
====
+
[NOTE]
====
...
This will suit our needs but will sacrifice higher data redundancy. The general guidance here is to use two or three volume replicas, but always one less than the number of nodes in the {rke2} cluster.
====
...
[NOTE]
====
The version used for this project is {cert-manager-version}. If this version isn't displayed at the top, select it from the list on the right.
====
+
