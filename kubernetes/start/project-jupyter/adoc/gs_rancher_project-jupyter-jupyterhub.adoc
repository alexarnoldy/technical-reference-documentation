...
:harvester-cloud-config-template-name:``
...
{lets-encrypt} will require that you provide the credentials to make changes to a publicly resolvable DNS domain, therefore you must also possess a publicly registered DNS domain with API credentials to add and remove records from it. 
+
[CAUTION]
====
...
You will need this for Let's Encrypt to verify your ownership of the domain supporting the {jupyterhub} service and provide the required TLS certificate for HTTPS communication. 
See this link:https://community.letsencrypt.org/t/dns-providers-who-easily-integrate-with-lets-encrypt-dns-validation/86438[blog post] for a general list of supported DNS providers.
For the purposes of this document, we will be using a domain based in {route53-name}.
...
----
+
.. Copy all of these keys into the `Key` column. Each key will automatically get populated into its own row.
+
[source, console]
----
AWS_ACCESS_KEY_ID
...
[IMPORTANT]
====
Any keys that do not have values assigned to them will be omitted when the ConfigMap is created. This missing data will cause errors later in this procedure.
====
+
...
This is needed to activate the "kernel-default" kernel. 
...
[NOTE]
====
(Optional) If additional load balanced IP addresses are desired, complete the following two steps.
====
+
. Create these shell variables:
[source, console, subs="attributes+"]
----
...
== Deploying {cert-manager} on {rke2-full}
...
[source, console, subs="attributes+"]
----
kubectl apply -f https://github.com/cert-manager/cert-manager/releases/download/{cert-manager-version}/cert-manager.crds.yaml
----
+
. Close the {rancher-kubectl-shell} and move on to the next step of this procedure.
...
== Creating the {jupyterhub} TLS certificate and private key through {cert-manager}

. Add the dedicated IP address configuration to {cert-manager}:
.. While viewing the {jupyterhub-deployed-name} cluster, select the {rancher-kubectl-shell} icon image::
+
[NOTE]
====
...
export DNSZONE="" # E.g. "susealliances.com"
export FQDN="" # E.g. "jupyterhub.susealliances.com"
export EMAIL_ADDR="" # valid email address for the Let's Encrypt certificate
----
+
. Ensure the fully qualified domain name of the {jupyterhub-deployed-name} service resolves to its dedicated IP address.
+
[source, console, subs="attributes+"]
----
getent hosts ${FQDN}
...
[source, console, subs="attributes+"]
----
kubectl describe -n {jupyterhub-namespace} certificate {jupyterhub-deployed-name}-certificate
----
+
[IMPORTANT]
====
...
[source, console, subs="attributes+"]
----
kubectl -n {jupyterhub-namespace} delete secret {jupyterhub-deployed-name}-certificate-secret
kubectl -n {jupyterhub-namespace} delete certificate {jupyterhub-deployed-name}-certificate
----
+
...
metadata:
  name: {jupyterhub-deployed-name}-certificate
  namespace: {jupyterhub-namespace}/expo
spec: 
  secretName: {jupyterhub-deployed-name}-certificate-secret # Kubernetes secret that will contain the tls.key and tls.crt of the new cert
  commonName: ${FQDN}
  dnsNames:
...
== Deploying {jupyterhub} on {rke2-full}
