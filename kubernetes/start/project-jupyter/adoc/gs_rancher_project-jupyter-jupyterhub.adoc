...
You can easily obtain a domain name from most public DNS providers for a small fee, though using a provider other than {route53-name} will mean some of the procedures outlined here will be different.
+
[CAUTION]
====
...
. If the {rancher-kubectl-shell} isn't already open and "Connected", you can open it while viewing the {jupyterhub-deployed-name} cluster by selecting the {rancher-kubectl-shell} icon image::rancher-kubectl-shell.png[Kubectl Shell] at the top of the window.
...
. If you want to, you can pull the border between the {rancher-kubectl-shell} image::rancher-pull-button.png[Rancher Pull Button] and the main {rancher} UI down until you need the shell again.
...
. You can press "Shift+Enter" to run the selected cell and advance to the next cell, or click the equivilent button at the top of the tab image::juypterhub-run-cell-and-advance.png[JupyterHub Run Cell and Advance]
