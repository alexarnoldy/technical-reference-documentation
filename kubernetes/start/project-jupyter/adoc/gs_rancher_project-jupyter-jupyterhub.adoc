...
kind: Certificate
metadata:
  name: {jupyterhub}-certificate
  namespace: {jupyterhub-namespace}
spec: 
  secretName: {jupyterhub}-certificate-secret # Kubernetes secret that will contain the tls.key and tls.crt of the new cert
  commonName: ${FQDN}
  dnsNames:
    - ${FQDN}
  issuerRef:
...
[source, console, subs="attributes+"]
----
kubectl describe -n {jupyterhub-namespace} certificate {jupyterhub}-certificate
----
+
[IMPORTANT]
====
...
[source, console, subs="attributes+"]
----
kubectl -n istio-system delete secret {jupyterhub}-certificate-secret
kubectl -n istio-system delete certificate {jupyterhub}-certificate
----
