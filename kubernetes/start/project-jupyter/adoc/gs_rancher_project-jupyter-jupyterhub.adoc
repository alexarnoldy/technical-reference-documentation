...
* Select the checkbox for `Install guest agent`
...
[TIP]
====
It is beyond the scope of this document to manage the {rke2} cluster with third party tools (e.g. kubectl). However, if you have a workstation that meets the requirements described link:https://longhorn.io/docs/1.5.3/deploy/install/#using-the-environm
ent-check-script[in this document], you can use the script contained there within to verify the {longhorn} nodes meet the requirements before deploying {longhorn}
====
+
[NOTE]
====
In deploying {longhorn}, we have used these specific parameters:
