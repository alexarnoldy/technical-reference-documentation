...
. A {harvester} cluster as the infrastructure provider for the {rke2} cluster.
//
+
[NOTE]
====
...
//You will need this for Let's Encrypt to verify your ownership of the {juypterhub} service and provide the required TLS certificates for HTTPS communication. 
...
You'll deploy {longhorn} for highly available, persistent storage.
+
...
+
* Installing {juypterhub}: Using its Helm chart, we'll seamlessly integrate {juypterhub} within your RKE2 cluster, equipping you with a dynamic platform for interactive data analysis and collaboration.
