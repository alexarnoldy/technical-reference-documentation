...
{lets-encrypt} will require that you possess the credentials to make changes to a publicly resolvable domain name.
+
[NOTE]
====
...
Most of the required software can be installed on {sles-full} can be installed with this command:
+
[source, console]
----
...
Download SLES15-SP5-Minimal-VM.x86_64-Cloud-GM.qcow2 image  at https://www.suse.com/download/sles/ (may need to log in or create a free account)

Upload the image IAW https://docs.harvesterhci.io/v1.3/upload-image

Create an RKE2 cluster IAW https://docs.harvesterhci.io/v1.3/rancher/node/rke2-cluster/#create-rke2-kubernetes-cluster

For the purposes of this project, the following `User Data` was used:
+
[NOTE]
====
You can change the root and sles user password, if you prefer.
====
+
...
chpasswd:
  ...
  expire: false
runcmd:
  - SUSEConnect --url http://rmt.stable.suse.lab
  - SUSEConnect -p sle-module-containers/{sles-version}.{sles-sp}/x86_64
  - zypper -n in -t pattern apparmor
  - zypper -n in iptables
  - zypper -n up
...
[TIP]
====
This `User Data` can also be configured as a Cloud Config Template in the {harvester} UI. See the link:https://docs.harvesterhci.io/v1.3/vm/index#cloud-configuration[{harvester} documentation] for more information.
====

Under `Cluster Configuration`, select the `Kuberntes Version` of :rke2-version:
