...
.. While viewing the {jupyterhub-deployed-name} cluster, select the {rancher-kubectl-shell} icon image::
+
.. When the {rancher-kubectl-shell} is ready, ensure the fully qualified domain name of the {jupyterhub-deployed-name} service resolves to its dedicated IP address.
+
[source, console, subs="attributes+"]
----
export JUPYTERHUB_FQDN=""
----
+
[source, console, subs="attributes+"]
----
getent hosts ${JUPYTERHUB_FQDN}
----
+
. Create these shell variables:
+
...
export DNSZONE="" # E.g. "suseaaliances.com"
...
[NOTE]
====
When initially creating the cert-manager Issuer, ensure the `server: https://acme-staging-v02` line is uncommented and the `server: https://acme-v02` line is commented out. 
