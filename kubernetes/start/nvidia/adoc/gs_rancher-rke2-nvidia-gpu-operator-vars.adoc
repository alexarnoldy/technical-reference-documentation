...
:sles-docs-sudo: {sles-docs}single-html/SLES-administration/#cha-adm-sudo
...
:description: Deploy {nvidia-op} with {sles-full} and {rke2-full} for Kubernetes worker nodes installed with the {nvidia-drv}.

:description-social: GPU-enabled Kubernetes clusters with SUSE

:executive-summary: Enable {nvidia-org} GPU resources for containerized workloads on SUSE {rke2-full} ({rke2}) clusters.
...
:contrib2-jobtitle: Solution Engineer, Software Systems Engineering
:contrib2-orgname: SUSE
:editor1-firstname: Terry
:editor1-surname: Smith
:editor1-jobtitle: Director, Global Alliance Solutions
