...
The remainder of this process will be completed through the {rancher} UI. Leveraging the {rancher} UI is more secure than managing {k8s} clusters from a workstation. Authentication is required, and there is no Kubeconfig file needed that can fall into the wrong hands. In addition, command line sessions are non-persistent.
