...
:sles-version: 15
:sles-sp: 5
:sles-kernel-version: 5.14.21-150500.55.49.1
...
//
+
Enable the Desktop Applications, and Development Tools {sles-docs-modules}[modules] on the build node:
+
[source, console, subs="attributes+"]
----
VER={sles-version}
...
* `OPERATOR_VERSION`: The associated {nvidia-op-url}platform-support.html[{nvidia-op}] version.
...
//+
* `KERNEL_VERSION`: Use the following command to find the Linux kernel version installed on all of the GPU-enabled worker nodes:
+
[source, console, subs="attributes+"]
----
sudo zypper se -s --installed-only kernel-default
----
+
[NOTE]
====
