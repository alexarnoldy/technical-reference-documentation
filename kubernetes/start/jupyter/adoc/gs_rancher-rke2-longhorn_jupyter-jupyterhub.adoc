...
The general guidance here is to use two or three volume replicas, but always one less than the number of nodes in the {rke2} cluster.
====
...
====
+
. After the certificate has been successfully issued, edit the `letsencrypt-issuer` to change from the {lets-encrypt} staging server to the production server:
+
[source, console, subs="attributes+"]
----
