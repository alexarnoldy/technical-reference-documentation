...
:comp1-provider: SUSE
:comp1: Rancher 
:comp1-full: Rancher Prime 
:comp1-version: v2.8.2
...
:comp2-provider: SUSE
:comp2: RKE2
:comp2-full: Rancher Kubernetes Engine 2
:comp2-version: v1.27.14+rke2r1
:comp2-website: https://www.suse.com/products/rancher-kubernetes-engine/
:comp2-docs: https://docs.rke2.io/

:comp3-provider: SUSE
:comp3: Longhorn
:comp3-full: Longhorn
:comp3-version: 103.3.1+up1.6.2
:comp3-website: https://longhorn.io/
:comp3-docs: https://longhorn.io/docs/latest

:comp4-provider: Jupyter
:comp4: JupyterHub
:comp4-full: JupyterHub
:comp4-version: 4.0.2
:comp4-website: https://jupyter.org/hub
:comp4-docs: https://jupyterhub.readthedocs.io/en/latest/
...
:rancher-kubectl-shell-variables-file: /tmp/jupyterhub-variables
:rancher-variables-configmap-name: jupyterhub-variables

:longhorn: Longhorn
:longhorn-version: 103.3.0+up1.6.1
...
:harvester-cloud-configuration-url: https://docs.harvesterhci.io/v1.3/vm/index#cloud-configuration
...
:suse-contact-url: https://www.suse.com/contact/
...
:rmt-name-full: SUSE Repository Mirroring Tool
:scc-name-full: SUSE Customer Center

:app-collection: Application Collection
:app-collection-getting-started-url: https://docs.apps.rancher.io/get-started/authentication/
...
:jupyterhub-chart-version: 3.3.7
...
//
+
{harvester} will be the infrastructure provider for the {rke2-full} cluster. 
{rancher} must be configured with cloud credentials with full access the {harvester}. 
...
If you don't have a {harvester} cluster readily available, {rke2} can be deployed on another infrastructure provider, or a hosted {k8s} can be used, though some of the procedures outlined here will be different.
====
+
...
[TIP]
====
In our demonstration deployment, our {jupyterhub} installation is behind a corporate firewall/NAT router.
Thus we are using non-routable IP addresses. 
This means our clients will need to have access to the public internet to resolve the {jupyterhub} fully qualified domain name through {route53-name} *AND* VPN access to connect to our {jupyterhub} service's IP address.
====
+
...
+
* *Publicly resolvable DNS domain name*
+
This guide will focus on using {route53-name} to provide a Fully Qualified Domain Name (FQDN) for the {jupyterhub} service. 
Let's Encrypt will require that you provide API credentials that allow adding and removing records from your DNS domain.
This is to verify your ownership of the domain. 
See this link:{lets-encrypt-dns-providers-url}[blog post] for a general list of supported DNS providers.
You may easily obtain a domain name from most public DNS providers for a small fee, though using a provider other than {route53-name} will mean some of the procedures outlined here will be different.
+
[CAUTION]
====
It is HIGHLY recommended that the API credentials used in this procedure possess no additional capabilities beyond adding/removing records from the specified domain.
====
+
* *RMT server or SUSE Linux subscription license*
+
You will deploy multiple VMs with {sles-full}. 
In order to obtain the proper updates and software, you will need to register these VMs through a {rmt-name-full} or with a {scc-name-full}.
+
* *Rancher Prime Subscription and secure token*
You will deploy SUSE secured versions of open source applications from the Rancher Prime {app-collection}. 
These applications are rebased to a secure SUSE Base Container Image with most Common Vulnerabilities and Exposures (CVEs) resolved. 
You can use link:{app-collection-getting-started-url}[this link] to access the {app-collection} and a secure token. 
Save the token somewhere safe.
If you do not have access to the {app-collection}, use link:{suse-contact-url}[this link] to request access.
...
* Configuring {longhorn} to provide highly available, persistent storage.

* Configuring {metallb}: This versatile load balancer manages virtual IP addresses that ensures traffic to your {jupyterlab} server instances isn't interrupted, even in the face of VM/host failures.
...
Create a `Cloud Config Template` based on link:{harvester-cloud-configuration-url}[these instructions].
+
[NOTE]
====
...
[NOTE]
====
If you prefer, update the cloud-config template below to configure a more secure password for the root and sles users.
====
+
[IMPORTANT]
====
Uncomment and update ONLY ONE the `SUSEConnect` commands below to register the VM operating systems either via an {rmt-name-full} server URL or with a subscription license (i.e. regcode). 
Be sure to remove the other `SUSEConnect` command.
====
+
...
. *Open the {rancher} UI* and follow the instructions at link:{harvester-create-rke2-cluster-url}[this URL] to create an {rke2} cluster on {harvester}.
+
[TIP]
====
When selecting the `User Data` template, some browsers won't pull in the correct data if the `Install guest agent` checkbox is selected. 
To ensure a consistent experience, you may first unchecked that box, selected the correct user data, then checked that box again.
====
+
[NOTE]
====
...
Please continue this procedure only after the cluster status shows `Active` image::rancher-active-jupyterhub-cluster.png[Rancher Active JupyterHub Cluster]
...
{longhorn} cannot operate with the minimal "kernel-default-base" kernel that comes with the {sles-minimal-file-name} image.

. In the {rancher} UI, select {virtualization-management}, select the {harvester} cluster, then select `Virtual Machines`
. Select the three dots to the right of the first virtual machine in {jupyterhub} cluster, then select `Restart` image::rancher-restart-vm.png[Rancher Restart VM]
...
Certain information is needed multiple times throughout this deployment process.
...
. Create the `variables` configmap in the `default` namespace:
...
[source, console, subs="attributes+"]
----
{rancher-variables-configmap-name}
----
+
...
. If the {rancher-kubectl-shell} isn't already open and "Connected", you may open it while viewing the {jupyterhub-deployed-name} cluster by selecting the {rancher-kubectl-shell} icon image::rancher-kubectl-shell.png[Kubectl Shell] at the top of the window.
. When the {rancher-kubectl-shell} is ready, run the following command:
+
[source, console, subs="attributes+"]
----
kubectl get configmap -n default {rancher-variables-configmap-name} -o json | jq -r '.data | to_entries[] | "\(.key)=\(.value)"' | sed 's/^/export /' > {rancher-kubectl-shell-variables-file} && cat {rancher-kubectl-shell-variables-file}
----
...
[NOTE]
====
In deploying {longhorn} in the {jupyterhub} {rke2} cluster, we have opted for lower storage consumption by configuring {longhorn} to create only two volume replicas per persistent volume. 
This will suit our needs but will sacrifice higher data redundancy. 
The general guidance here is to use two or three volume replicas, but always one less than the number of {longhorn} enabled nodes in the {rke2} cluster.
====
...
[TIP]
====
After the final message `SUCCESS: helm upgrade --install=true...` is displayed, you may close the `Install longhorn-system:longhorn` terminal window image::rancher-install-longhorn-terminal.png[Install Longhorn Terminal] and continue with the next steps in this procedure.
====
+
. Finish configuring {longhorn} and verify proper functionality

.. From the menu on the left, select `Storage`, then `StorageClasses`
.. If any StorageClasses, aside from the `Longhorn` StorageClass, show as a "Default", click on the three dots to the right of each one, then select `Reset Default` image::reset-default.png[Reset Default]
...
[TIP]
====
After the final message `SUCCESS: helm install...` is displayed, you may close the `Install metallb-system:metallb` terminal window image::rancher-install-metallb-terminal.png[Install MetalLB Terminal] and continue with the next steps in this procedure.
====
+
[NOTE]
====
With the following step you will add the dedicated IP address configuration to {metallb}.
====
+
. While viewing the {jupyterhub-deployed-name} cluster, select the {rancher-kubectl-shell} icon image::rancher-kubectl-shell.png[Kubectl Shell]
...
[TIP]
====
There are many ways to specify the desired IP address range. 
See the link:{metallb-ipaddress-config-url}[{metallb} documentation] for more information.
====
+
...
[NOTE]
====
With the following commands you will validate that {metallb} provides an IP address to an application and that {longhorn} provides a persistent volume, when they are called upon.
====
+
. Paste this command into the {rancher-kubectl-shell}:
+
...
----
+
. Verify the pod is `Running`, the longhorn StorageClass is the only `(default)` StorageClass, the persistentvolumeclaim is `Bound`, and the service has an `EXTERNAL-IP` (Repeat the command until the pod shows a status of `Running`)
+
[source, console, subs="attributes+"]
----
...
[TIP]
====
If you want, you may pull the border between the {rancher-kubectl-shell} and the main {rancher} UI down until you need the shell again.
====
...
Then, you will deploy the {cert-manager} Helm chart.
...
[NOTE]
====
The version used for this project is {cert-manager-version}. 
If this version isn't displayed at the top, select it from the list on the right.
====
+
...
[TIP]
====
After the final message `SUCCESS: helm install...` is displayed, you may close the `Install cert-manager:cert-manager` terminal window image::rancher-install-cert-manager-terminal.png[Install Cert-Manager Terminal] and continue with the next steps in this procedure.
====
...
[NOTE]
====
When initially creating the cert-manager Issuer, you will use the *{lets-encrypt-base-staging-url}* server to test the process of issuing a certificate to avoid the Let's Encrypt production rate limits. 
After verifying that the certificate can be issued correctly, we will update to the *{lets-encrypt-base-prod-url}* server to obtain the valid, production certificate.
====
+
...
spec:
  acme:
    email: ${EMAIL_ADDR}
    server: {lets-encrypt-full-staging-url} 
    privateKeySecretRef:
...
====
+
. After the certificate has been successfully issued, update the `letsencrypt-issuer` resource to change from the {lets-encrypt} staging server to the production server:
+
[source, console, subs="attributes+"]
----
source /tmp/jupyterhub-variables && \
kubectl patch issuer letsencrypt-issuer -n jupyterhub --type=json -p='[{"op": "replace", "path": "/spec/acme/server", "value": "https://acme-v02.api.letsencrypt.org/directory"}]'
----
+
////
[NOTE]
...
[source, console, subs="attributes+"]
----
kubectl get -n jupyterhub secrets
----
. Check the status of the certificate:
+
[source, console, subs="attributes+"]
...
.. Use `Ctrl+c` to exit the kubectl -w (watch) command
+
. Continue on to the next step in this process only after the production certificate has been issued.
...
See the {jupyterhub-auth-doc}[{jupyterhub} authentication documentation] for information on integrating {jupyterhub} with a central authencation application.
====
...
Specifically, you must possess a DNS domain registered with a public DNS provider. 
In this guide we will use the Fully Qualified Domain Name of {public-fqdn}.
====
...
helm upgrade --cleanup-on-fail \
  ...
  --create-namespace \
  --version={jupyterhub-chart-version} \
  --values config.yaml
----
...
. Connect to your deployed {jupyterhub} fully qualified domain name. In our example it is https://{public-fqdn}.

. Log in with your username
+
[NOTE]
====
If an identity provider hasn't been specifically integrated with {jupyterhub}, it will accept any username as a log in and does not require a password.
====
+
. From the {jupyterlab} Launcher, select the `Terminal` session image::jupyterhub-terminal.png[JupyterLab Terminal]
...
----
+
. After the repository has been downloaded, use the file browser tool at the top left of the window to navigate to `notebook` -> `docs` -> `source` -> `examples` -> `Notebook` image::jupyterhub-notebook-file-browser.png[JupyterLab notebook in File Browser].
...
. You may press "Shift+Enter" to run the selected cell and advance to the next cell, or click the equivilent button at the top of the tab image::juypterhub-run-cell-and-advance.png[JupyterHub Run Cell and Advance].
...
In this document you have gone through the process of configuring {harvester} to create the VMs needed for your {jupyterhub} deployment, deployed {rke2-full} onto those VMs, then added {longhorn}, {metallb}, and {cert-manager}.
Finally, you have installed {jupyterhub} and verified that the installation is working correctly. 
Additional customizations are outside of the scope of this document. 
